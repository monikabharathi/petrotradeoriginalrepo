...
@Component({
  ...
  styleUrls: ['./authorizeuser.component.css']
})
export class Authorizeuser implements OnInit {
  filter:any;
  config: any;
  parentmenu: any = {};
  collection = { data: [] };
  ...
  getProfileAuthrized(){
    ...
    this.rest.sendPostRequest('usersWatingforauth',JSON.stringify(postData))
      .subscribe(
...
initPagination(){
 ...
 ViewAuthUser(data) {
  ...
  this.router.navigate(['/layout/viewauthorizeuser']);
 }
