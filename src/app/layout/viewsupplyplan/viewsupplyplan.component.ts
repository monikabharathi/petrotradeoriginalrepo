...
@Component({
  selector: 'app-viewsupplyplan',
  templateUrl: './viewsupplyplan.component.html',
  styleUrls: ['./viewsupplyplan.component.css'],
  providers: [
    DatePipe
  ]
})
export class ViewsupplyplanComponent implements OnInit {
  issubmit: any;
  msg : any;
  rowNum: any;
  sno: any;
...
showTotalfun(option){
 ...
 this.setPurchaseSummaryHeaders();
    ...
    this.colums.forEach((key)=>{ 
      if(!key.includes('Date') && !key.includes('flag') && !key.includes('boxfiled') && !key.includes('bulkstock') && key.includes('LeadUpdate') && !key.includes('color') && !key.includes('Sales') && !key.includes('WEEKDAY')){
          this.displayedColumns.push(key);
      }
    });
                        ...
                        let valueObj = key3 ? key3[colKey] : '';
                          for(let key4 in valueObj){
                            for(let key5 in valueObj[key4]){
                              ...
                              if((key5 != 'Opening') && (key5 != 'WEEKDAY') && (key5 != 'SAFTEYDAYS')){
                                let allDate = new Date(key2);

                                console.log("allDate : "+allDate);
                                console.log("Sys Date : "+this.systemDate);
                    
                                tempObj[key+'_'+colKey+'_'+key5+'_flag'] = true;//allDate <= this.systemDate ? true : false;
                          
                           // tempObj[key+'_'+colKey+'_'+key5+'_flag'] = this.datePipe.transform(allDate, "yyyy-MM-dd") < this.datePipe.transform(this.systemDate, "yyyy-MM-dd") ? true : false;
                                }
                            }
                          }
...
onBack(){
 ...
 getBackgroundColor(columnName, data,seldate,row){
  ...
  //column.split('_')[2]=='Opening' || !(column.split('_')[0]=='SupplyPlan')
   ...
   // console.log("data in get background color "+data);
  return color;
  }
